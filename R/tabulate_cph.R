...
##' @param data_labels
##'
tabulate_cph <- function(fit, data, data_labels) {

  tbl1_stats <- data[, data_labels$variable_original] %>%
    tibble_one(formula = ~ .,
  ...
  miss_stats <- miss_var_summary(data[, data_labels$variable_original]) %>%
    transmute(
      variable_original = variable,
  ...
  unadjusted_hr <- function(variable_name, data){
    ...
    tidy(coxph(formula, data), exponentiate = TRUE)

  }

  unadjusted_hrs <- intersect(data_labels$variable, fit$term) %>%
    map_dfr(.f = unadjusted_hr, data = trn)

  models <- list("mdl_one" = unadjusted_hrs, 'mdl_two' = fit)
