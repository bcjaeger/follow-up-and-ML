...
the_plan <- drake_plan(
  ...
  data_labels = make_labels(),

  xgb_params = list(
    eta                = 0.01,
    num_parallel_trees = 10,
    max_depth          = 3,
    gamma              = 1/3,
    min_child_weight   = 1,
    subsample          = 1/2,
  ...
  fig_cv_m1  = make_fig_cv_blocks(cv_tidy,
                                  models = c('ref', 'xgb', 'xgb_cph'),
                                  times = c('m0.00', 'm1.00'),
                                  add_squares = TRUE),

  fig_cv_w1  = make_fig_cv_blocks(cv_tidy,
                                  models = c('ref', 'xgb', 'xgb_cph'),
                                  times = c('m0.00', 'm0.25'),
                                  add_squares = TRUE),
  ...
  tbl_fit_final = tabulate_cph(fit = final_fit_combined[[3]]$fit,
                               data,
                               data_labels),
